$(document).ready(function () {
  ...
  function displayValues(datas) {
    ...
    for (let j = 0; j < datas.length; j++) {
      ...
      const image = $("<img />", {
        id: "breed_image",
        src: storedBreeds[datas[j]].image.url,
      });

      const breedName = $("<h2 />", {
        id: "breed_name",
        class: "show",
        html: storedBreeds[datas[j]].name,
      });

      const temperament = $("<p />", {
        id: "temperament",
        class: "show",
        html: storedBreeds[datas[j]].temperament,
      });

      const description = $("<p />", {
        id: "description",
        class: "hidden",
        html: storedBreeds[datas[j]].description,
      });
      ...
      const url = $("<a />", {
        ...
        target: "_blank",
        html: "Learn More",
        href: storedBreeds[datas[j]].wikipedia_url,
      });
